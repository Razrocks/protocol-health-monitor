"""
Load raw DeFiLlama data into PostgreSQL
"""
import os
import json
import psycopg2
    ...
    def parse_and_load_tvl_history(self, protocol_id: int, raw_data: Dict):
        ...
        for point in tvl_data:
            if isinstance(point, dict) and 'date' in point and 'totalLiquidityUSD' in point:
                # DeFiLlama returns date as Unix timestamp
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python load_raw_to_pg.py <path_to_json>")
        sys.exit(1)
    ...
    try:
        run_id = loader.create_ingestion_run()
        print(f"Created run_id: {run_id}")
        
        success_count = 0
        for slug, raw_data in data.items():
            if loader.load_protocol_data(run_id, slug, raw_data):
                success_count += 1
        
        loader.update_ingestion_run(
            run_id,
            'success',
            f"Loaded {success_count}/{len(data)} protocols"
        )
        
        print(f"\nCompleted: {success_count}/{len(data)} protocols loaded")
